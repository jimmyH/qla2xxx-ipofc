...
qla25xx_fdmi_port_speed_capability(struct qla_hw_data *ha)
{
	...
	if (IS_QLA27XX(ha)) {
		if (FW_ABILITY_MAX_SPEED(ha) == FW_ABILITY_MAX_SPEED_32G)
			return FDMI_PORT_SPEED_32GB|FDMI_PORT_SPEED_16GB|
			       FDMI_PORT_SPEED_8GB;
...
void qla2x00_async_gpnid_sp_done(void *v, void *s, int res)
{
	struct scsi_qla_host *vha = (struct scsi_qla_host*)v;
	struct srb *sp = (struct srb *)s;
	struct ct_sns_req *ct_req =(struct ct_sns_req*)sp->u.iocb_cmd.u.ctarg.req;
	struct ct_sns_rsp *ct_rsp =(struct ct_sns_rsp*)sp->u.iocb_cmd.u.ctarg.rsp;
	struct event_arg ea;
	struct qla_work_evt *e;
