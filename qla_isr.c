 ...
 */
QLA_IRQ_HANDLER(qla2100_intr_handler)
{
	scsi_qla_host_t	*ha;
 ...
 * Returns handled flag.
 */
QLA_IRQ_HANDLER(qla2300_intr_handler)
{
	scsi_qla_host_t	*ha;
	device_reg_t __iomem *reg;
	int		status;
...
qla2x00_error_entry(scsi_qla_host_t *ha, sts_entry_t *pkt)
{
	...
	printk("scsi(%ld): Packet:\n", ha->host_no);
	qla2x00_dump_buffer((uint8_t *)pkt, sizeof(sts_entry_t));
#endif
 ...
 * Returns handled flag.
 */
QLA_IRQ_HANDLER(qla24xx_intr_handler)
{
	scsi_qla_host_t	*ha;
	struct device_reg_24xx __iomem *reg;
	int		status;
