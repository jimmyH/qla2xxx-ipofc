...
skip_rio:
	...
	case MBA_LOOP_DOWN:		/* Loop Down Event */
		...
		if (atomic_read(&vha->loop_state) != LOOP_DOWN) {
			 ...
			 * Restore Physical port WWPN only.
			 */
			if (!vha->vp_idx) {
    ...
    struct mbx_entry *mbx)
{
	...
	ql_log(ql_log_warn, vha, 0x5046,
	    ...
	    fcport->d_id.b24, status, le16_to_cpu(mbx->mb0),
	    le16_to_cpu(mbx->mb1), le16_to_cpu(mbx->mb2),
	    le16_to_cpu(mbx->mb6), le16_to_cpu(mbx->mb7));
    ...
    struct logio_entry_24xx *logio)
{
	...
	if (logio->entry_status) {
		ql_log(ql_log_warn, fcport->vha, 0x5034,
		    ...
		    "portid=%06x entry-status=%x.\n",
		    type, sp->handle, fcport->d_id.b24, logio->entry_status);
	...
	if (le16_to_cpu(logio->comp_status) == CS_COMPLETE) {
		ql_dbg(ql_dbg_async, fcport->vha, 0x5036,
		    "Async-%s complete - hdl=%x portid=%06x "
		    "iop0=%x.\n", type, sp->handle, fcport->d_id.b24,
	...
	ql_dbg(ql_dbg_async, fcport->vha, 0x5037,
	    "Async-%s failed - hdl=%x portid=%06x comp=%x "
	    "iop0=%x iop1=%x.\n", type, sp->handle, fcport->d_id.b24,
...
out:
	if (logit)
		ql_dbg(ql_dbg_io, fcport->vha, 0x3022,
		    ...
		    "nexus=%ld:%d:%d portid=%06x oxid=0x%x "
		    "cdb=%10phN len=0x%x "
		    "rsp_info=0x%x resid=0x%x fw_resid=0x%x sp=%p cp=%p.\n",
		    comp_status, scsi_status, res, vha->host_no,
		    cp->device->id, cp->device->lun, fcport->d_id.b24, ox_id,
