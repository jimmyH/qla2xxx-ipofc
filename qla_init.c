...
static void qla24xx_handle_plogi_done_event(struct scsi_qla_host *,
	struct event_arg*);
static void qlt_logo_completion_handler(fc_port_t *fcport, int rc);
	...
	struct event_arg *ea)
{
	...
	for (i=0; i < n; i++) {
		...
		default:
			...
			ql_dbg(ql_dbg_disc, vha, 0xffff,
				"%s %d %8phC handle login cur LS %d\n",
				__func__, __LINE__, fcport->port_name,
				e->current_login_state);
		...
		if (fcport->loop_id == FC_NO_LOOP_ID) {
			...
			for (i=0; i < n; i++) {
				...
				if (fcport->d_id.b24 == id.b24) {
					conflict_fcport =
						qla2x00_find_fcport_by_wwpn(vha,e->port_name,0);
					ql_dbg(ql_dbg_disc, vha, 0xffff,
						"%s %d %8phC post del sess\n",
						__func__, __LINE__, conflict_fcport->port_name);
					qlt_schedule_sess_for_deletion(conflict_fcport);
				}
...
qla24xx_async_gnl_sp_done(void *v, void *s, int res)
{
	...
	if (sp->u.iocb_cmd.u.mbx.in_mb[1] >=
		...
		n = sp->u.iocb_cmd.u.mbx.in_mb[1]/ sizeof(struct get_name_list_extended);
...
qla24xx_reset_risc(scsi_qla_host_t *vha)
{
	...
	ql_dbg(ql_dbg_init + ql_dbg_verbose, vha, 0x0193,
	...
	ql_dbg(ql_dbg_init + ql_dbg_verbose, vha, 0x0194,
...
qla2x00_init_rings(scsi_qla_host_t *vha)
{
	...
	if (IS_FWI2_CAPABLE(ha)) {
		...
		ql_dbg(ql_dbg_init, vha, 0x0192, "FA-WWPN Support: %s.\n",
...
qla2x00_rport_del(void *data)
{
	...
	if (rport) {
		ql_dbg(ql_dbg_disc, fcport->vha, 0xffff,
		    ...
		    __func__, fcport->port_name, rport, rport->roles);
		...
		if (mb[0] == MBS_PORT_ID_USED) {
			...
			*next_loopid = fcport->loop_id;
			...
			ql_dbg(ql_dbg_disc, vha, 0x2002,
			    "Failed=%x port_id=%06x loop_id=%x "
			    "jiffies=%lx.\n", mb[0], fcport->d_id.b24,
...
void qla24xx_chk_fcp_state(struct fc_port *sess)
{
...
/* ha->vport_slock supposed to be held on entry */
...
static void qlt_free_session_done(struct fc_port *sess)
{
	...
	ql_dbg(ql_dbg_disc, vha, 0xf084,
		"%s: sess %p from port %8phC loop_id %#04x"
		" s_id %02x:%02x:%02x logout %d keep %d\n",
		__func__, sess, sess->port_name, sess->loop_id,
...
void qla24xx_delete_sess_fn(struct work_struct *work)
{
	...
	ql_dbg(ql_dbg_disc, fcport->vha, 0xffff,
		   "%s fcport %p for deletion %8phC\n",
		   __func__, fcport, fcport->port_name);

	spin_lock_irqsave(&ha->vport_slock, flags);
	qla2x00_mark_device_lost(fcport->vha, fcport, 1, 0);

	fcport->deleted = QLA_SESS_DELETION_IN_PROGRESS;
	fcport->disc_state = DSC_DELETE_PEND;
	fcport->last_rscn_gen = fcport->rscn_gen;
	fcport->last_login_gen = fcport->login_gen;
	spin_unlock_irqrestore(&ha->vport_slock, flags);

	qlt_free_session_done(fcport);
}
...
void qlt_logo_completion_handler(fc_port_t *fcport, int rc)
{
	if (rc != MBS_COMMAND_COMPLETE) {
		ql_dbg(ql_dbg_disc, fcport->vha, 0xf093,
			"%s: fcport %p from"
