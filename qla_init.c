...
qla2x00_tgt_alloc(scsi_qla_host_t *ha, uint16_t tgt)
{
	...
	tq = TGT_Q(ha, tgt);
	if (tq == NULL) {
		tq = kmalloc(sizeof(os_tgt_t), GFP_KERNEL);
		if (tq != NULL) {
...
qla2x00_lun_alloc(scsi_qla_host_t *ha, uint16_t tgt, uint16_t lun)
{
	...
	lq = LUN_Q(ha, tgt, lun);
	if (lq == NULL) {
		lq = kmalloc(sizeof(os_lun_t), GFP_KERNEL);
		if (lq != NULL) {
    ...
    fc_port_t *fcport, uint16_t tgt, uint16_t dev_no)
{
	...
	int		rval, lun, bit, idx;
	uint8_t         lun_mask[LUN_CHUNK_SIZE >> 3]; /* 32 bytes */
	lun_bit_mask_t  *mask_ptr;

	mask_ptr = (lun_bit_mask_t *)lun_mask;

	bit = 0;
	memset(&fcport->lun_mask, 0, sizeof(lun_bit_mask_t));

	/* Use LUN mask */
	for (idx = (TOTAL_256_LUN_CHUNKS - 1); idx >= 0; idx--) {
		...
		if (idx == 0) {
			...
			lun = idx *LUN_CHUNK_SIZE;
			bit = lun;
			sprintf(propbuf,
			    "scsi-qla%ld-tgt-%d-di-%d-lun_disabled-%d",
			    ha->instance, tgt, dev_no, lun);
		}

		rval = qla2x00_get_prop_xstr(ha, propbuf,
		    (uint8_t *)mask_ptr, (LUN_CHUNK_SIZE >> 3));
		if (rval == (LUN_CHUNK_SIZE >> 3)) {
			for (lun = (LUN_CHUNK_SIZE - 1); lun >= 0;
			    lun--, bit++) {
...
qla24xx_nvram_config(scsi_qla_host_t *ha)
{
	...
	if ((IS_QLA2422(ha) || IS_QLA2432(ha)) &&
	    timer_mode == (BIT_2 | BIT_1)) {
		qla_printk(KERN_INFO, ha,
