...
typedef struct _FO_PATHS_INFO
{
	...
	UINT8       CurrentPathIndex[1];   /* Variable size to support 4096 LUNs */
...
typedef struct _FO_EXTERNAL_LUN_DATA_ENTRY
{
	...
	UINT8       Data[1];    /* Varirble size to support 4096 LUNs */
...
#define FO_INTERNAL_LUN_COUNT		EXT_DEF_MAX_LUNS
#define FO_INTERNAL_LUN_BITMASK_BYTES  (FO_INTERNAL_LUN_COUNT / 8)

#define EXT_EXTERNAL_LUN_COUNT          EXT_DEF_MAX_LUNS
#define EXT_EXTERNAL_LUN_BITMASK_BYTES  (EXT_EXTERNAL_LUN_COUNT / 8)

typedef struct _FO_INTERNAL_LUN_BITMASK
{
	UINT8       Bitmask[1]; /* Variable length */
...
typedef struct _FO_TGT_LB_DATA_LIST
{
	UINT16       Reserved0;
	UINT16       EntryCount;
	UINT32       Reserved1;
...
} FO_TGT_LB_DATA_LIST, *PFO_TGT_LB_DATA_LIST;
...
#define FO_DEV_DATA_HEAD_SIZE offsetof(FO_DEVICE_DATA, LunBitmask)

#define FO_LUN_DATA_LIST_MIN_SIZE (FO_LUN_DATA_LIST_HEADER_SIZE +\
	((offsetof(FO_EXTERNAL_LUN_DATA_ENTRY, Data) + lunCount) *\
	FO_LUN_DATA_LIST_MIN_ENTRIES))

#define FO_LUN_DATA_LIST_MAX_SIZE (FO_LUN_DATA_LIST_HEADER_SIZE +\
	((offsetof(FO_EXTERNAL_LUN_DATA_ENTRY, Data) + lunCount) *\
	FO_LUN_DATA_LIST_MAX_ENTRIES))

#define PATH_ENTRY(Paths, Lun_count) (FO_PATH_ENTRY *)(Paths->CurrentPathIndex\
	+ Lun_count)
#endif	/* ifndef _FO_H */
